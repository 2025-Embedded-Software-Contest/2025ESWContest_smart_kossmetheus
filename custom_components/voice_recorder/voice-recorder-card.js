class VoiceRecorderCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    // ▼ 추가: 드롭다운 숨김/고정 이벤트명 옵션
    // this.hideEventSelect = config.hide_event_select === true;
    // this.fixedEventName = config.event_name || ""; // 숨길 때 서버로 보낼 고정값(선택)
  ...
  async initRecorder() {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.src =
        "https://cdn.jsdelivr.net/gh/kukuxx/Recorder@master/recorder.mp3.min.js";
      ...
      script.onload = () => {
        ...
        if (window.Recorder && window.Recorder.i18n) {
          ...
          i18nScript.src =
            "https://cdn.jsdelivr.net/gh/kukuxx/Recorder@master/src/i18n/en-US.js";
          i18nScript.onload = () => {
          ...
          i18nScript.onerror = () => {
            ...
            console.warn(
              "Failed to load English language pack, some text may still be in Chinese"
            );
            this._continueRecorderInit(resolve, reject);
          };
          ...
          for (let i = newBufferIdx; i < buffers.length; i++) {
            if (buffers[i] && buffers[i].length > 0) {
              ...
              for (let j = 0; j < originalBuffer.length; j++) {
                ...
                if (Math.abs(sample) > 16384) {
                  // 當超過一半範圍時開始軟限制
                  const sign = sample >= 0 ? 1 : -1;
                  const normalizedSample = Math.abs(sample) / 32767;
                  // 使用 tanh 函數進行軟限制
                  const limitedSample =
                    Math.tanh(normalizedSample * 2) * 32767 * sign;
      ...
      this.recorder.stop(async (blob, duration) => {
        try {
          ...
          const eventName = String(
            this.shadowRoot.querySelector("#eventnameInput").value || ""
          ).trim();
